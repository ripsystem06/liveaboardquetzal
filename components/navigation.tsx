      ...
      <div className={`container mx-auto px-4 lg:px-8 transition-all duration-300 ${isCompactDesktop ? 'h-10' : 'h-16'}`}>
        <div className={`flex items-center justify-between transition-all duration-300 ${isCompactDesktop ? 'h-10' : 'h-16'}`}>
          ...
          <div className={`hidden lg:flex items-center gap-3 transition-all duration-300 ${isCompactDesktop ? 'opacity-0 pointer-events-none absolute' : 'opacity-100'}`}>
            ...
            <Button asChild size="icon-sm" className="bg-muted text-muted-foreground hover:bg-accent/20 hover:text-accent-foreground" title={showAuthUI ? 'My Account' : t('booking.cruise.signIn')}>
              <Link href={showAuthUI ? '/account' : '/booking?step=1'}><User className="h-4 w-4" /></Link>
            </Button>
            {showAuthUI && (
          ...
          {isCompactDesktop && (
            <Sheet>
              ...
              <SheetContent side="top" className="bg-[#f3f1ec] text-foreground w-full max-h-[80vh] overflow-y-auto">
                <nav className="flex flex-col items-center gap-4 py-6">
                  <Link href="/" className="text-foreground hover:text-accent py-2 text-lg font-bold" onClick={() => {}}>
                    {t('nav.home')}
                  </Link>
                  <Link href="/nuestro-barco" className="text-foreground hover:text-accent py-2 text-lg font-bold">
                    {t('nav.ourBoat')}
                  </Link>
                  <div className="text-center space-y-2">
                    <p className="text-foreground font-bold text-lg">{t('nav.destinations')}</p>
                    <Link href="/destinos/islas-socorro" className="block text-foreground/80 hover:text-accent py-1 font-normal">
                      {t('nav.socorro')}
                    </Link>
                    <Link href="/destinos/mar-de-cortes" className="block text-foreground/80 hover:text-accent py-1 font-normal">
                      {t('nav.cortez')}
                    </Link>
                    <Link href="/destinos/bahia-magdalena" className="block text-foreground/80 hover:text-accent py-1 font-normal">
                      {t('nav.magbay')}
                    </Link>
                  </div>
                  <Link href="/faqs" className="text-foreground hover:text-accent py-2 text-lg font-bold">
                    {t('nav.faqs')}
                  </Link>
                  <div className="text-center space-y-2">
                    <p className="text-foreground font-bold text-lg">{t('nav.aboutUs')}</p>
                    <Link href="/about" className="block text-foreground/80 hover:text-accent py-1 font-normal">
                      {t('nav.aboutUs')}
                    </Link>
                    <Link href="/about/colaboraciones" className="block text-foreground/80 hover:text-accent py-1 font-normal">
        ...
        {isOpen && (
          <div className="lg:hidden pb-4 space-y-3 bg-[#f3f1ec]/95 backdrop-blur-sm text-center">
            <Link href="/" className="block text-foreground hover:text-accent py-2 font-bold" onClick={() => setIsOpen(false)}>
              {t('nav.home')}
            </Link>
            <Link href="/nuestro-barco" className="block text-foreground hover:text-accent py-2 font-bold" onClick={() => setIsOpen(false)}>
              {t('nav.ourBoat')}
            </Link>
            <div className="space-y-2">
              <p className="text-foreground font-bold">{t('nav.destinations')}</p>
              <Link href="/destinos/islas-socorro" className="block text-foreground/80 hover:text-accent py-1 font-normal" onClick={() => setIsOpen(false)}>
                {t('nav.socorro')}
              </Link>
              <Link href="/destinos/mar-de-cortes" className="block text-foreground/80 hover:text-accent py-1 font-normal" onClick={() => setIsOpen(false)}>
                {t('nav.cortez')}
              </Link>
              <Link href="/destinos/bahia-magdalena" className="block text-foreground/80 hover:text-accent py-1 font-normal" onClick={() => setIsOpen(false)}>
                {t('nav.magbay')}
              </Link>
            </div>
            <Link href="/faqs" className="block text-foreground hover:text-accent py-2 font-bold" onClick={() => setIsOpen(false)}>
              {t('nav.faqs')}
            </Link>
            <div className="space-y-2">
              <p className="text-foreground font-bold">{t('nav.aboutUs')}</p>
              <Link href="/about" className="block text-foreground/80 hover:text-accent py-1 font-normal" onClick={() => setIsOpen(false)}>
                {t('nav.aboutUs')}
              </Link>
              <Link href="/about/colaboraciones" className="block text-foreground/80 hover:text-accent py-1 font-normal" onClick={() => setIsOpen(false)}>
                {t('nav.collaborations')}
              </Link>
            </div>
            ...
            <div className="flex items-center justify-center gap-3 pt-4">
              {/* Profile icon — always visible */}
              <Button asChild size="icon-sm" className="bg-muted text-muted-foreground hover:bg-accent/20 hover:text-accent-foreground" title={showAuthUI ? 'My Account' : t('booking.cruise.signIn')}>
                <Link href={showAuthUI ? '/account' : '/booking?step=1'} onClick={() => setIsOpen(false)}><User className="h-4 w-4" /></Link>
              </Button>
              {showAuthUI && (
